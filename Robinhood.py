"""
Author: Matt Beattie (@mattbeattie)
Author: Milan Curcic (@milancurcic)
Author: Ryan D'souza (@dsouzarc)
Author: AJ Haggard (@ahaggard2013)
Author: Jamone Kelly (@jamonek)
Author: Rohan Pai (@rohanpai)
Author: Bharath Lohray (@lordloh)
...
class Robinhood:
    ...
    def __init__(self):
        ...
        self.session.proxies = urllib_request.getproxies()
        self.headers = {
            "Accept": "*/*",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "enq=1, frq=0.9, deq=0.8, jaq=0.7, nlq=0.6, itq=0.5",
    ...
    def login(self, username, password):
        ...
        data = urllib_parse.urlencode({"password": self.password, "username": self.username})
    ...
    def symbol_from_instrument(self, instrument):
        return self.session.get(instrument).json()['symbol']

    def instrument_positions(self):
        position = self.session.get(self.endpoints['positions']).json()['results']
        positions = []
        for stock in position:
            if float(stock['quantity']) > 0:
                positions += [stock['instrument']]
        return positions

    def symbol_positions(self):
        position = self.session.get(self.endpoints['positions']).json()['results']
        positions = []
        for stock in position:
            if float(stock['quantity']) > 0:
                positions += [self.symbol_from_instrument(stock['instrument'])]
        return positions
    ...
    def quote_data(self, stock=None):
        ...
        try:
            res = json.loads((urllib_request.urlopen(url)).read().decode('utf-8'))
            if len(res) > 0:
                return res
            else:
    ...
    def get_historical_quotes(self, symbol, interval, span, bounds='regular'):
        ...
        res = self.session.get(self.endpoints['historicals'], params={'symbols': ','.join(symbol).upper(),
                                                                      'interval': interval,
                                                                      'span': span,
                                                                      'bounds': bounds})
        return res.json()
    ...
    def place_order(self, instrument, quantity=1, bid_price=None, transaction=None):
        ...
        data = 'account=%s&instrument=%s&price=%f&quantity=%d&side=%s&symbol=%s&time_in_force=gfd&\
                trigger=immediate&type=market' % (
            self.get_account()['url'],
            urllib_request.unquote(instrument['url']),
            float(bid_price),
            quantity,
            transaction,
            instrument['symbol']
